...
exports.signup = async (req, res) => {
  try {
    ...
    const response = await OTP.find({ email: lowerCaseEmail })
      .sort({ createdAt: -1 })
      .limit(1);
    // otp not found
...
exports.sendotp = async (req, res) => {
  try {
    ...
    const otpBody = await OTP.create({ email: lowerCaseEmail, otp: otp });
    console.log("otp Body", otpBody.otp);
...
exports.changePassword = async (req, res) => {
  try {
    ...
    if (newPassword !== confirmNewPassword) {
      return res.status(400).json({
        success: false,
        message: "New Password and confirmNewPassword is not matching",
      });
    }
    // hash new password
